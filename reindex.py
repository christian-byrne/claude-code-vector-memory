...
def main():
    ...
    if not Path("requirements.txt").exists():
        console.print("[red]❌ Error: Not in the claude-code-vector-memory directory[/red]")
        sys.exit(1)
    
    # Check if venv is activated
    if not hasattr(sys, 'real_prefix') and not (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
        console.print("[yellow]⚠️  Virtual environment not activated[/yellow]")
        console.print("   Run: source venv/bin/activate (or venv\\Scripts\\activate on Windows)")
    ...
    if run_reindex():
        ...
        if backup_path:
            console.print(f"[yellow]Restore from backup: rm -rf chroma_db && mv {backup_path} chroma_db[/yellow]")
        sys.exit(1)
