...
class MetadataAdder:
    ...
    def extract_project_from_paths(self, content: str) -> Optional[str]:
        ...
        for pattern in project_patterns:
            ...
            if match:
                ...
                if project.startswith('comfyui-'):
                    return project
                elif 'comfyui' in project.lower():
                    return project
                elif project not in ['home', 'projects', 'user', 'Users']:
                    return project
    ...
    def estimate_complexity(self, content: str) -> str:
        ...
        command_count = len(re.findall(r'(?:npm|pip|git|python|node)', content))
    ...
    def extract_technologies(self, content: str) -> List[str]:
        ...
        return sorted(list(set(found_techs)))
    ...
    def extract_key_files(self, content: str) -> List[str]:
        ...
        rel_pattern = r'((?:src/|components/|tests/|scripts/)[^\s:]+(?:\.ts|\.js|\.vue|\.py))'
    ...
    def determine_outcome(self, content: str) -> str:
        ...
        if any(word in content_lower for word in ['error', 'failed', 'blocked', 'issue', 'problem']):
            # But check if they were resolved
            if any(word in content_lower for word in ['fixed', 'resolved', 'completed', 'success']):
                return "success"
            else:
                return "partial"
        
        # Check for success indicators
        if any(word in content_lower for word in ['completed', 'implemented', 'created', 'added']):
            return "success"
    ...
    def process_all_summaries(self) -> None:
        ...
        if results:
            ...
            console.print("[cyan]Run reindex.sh to update the search index with new metadata[/cyan]")
        else:
