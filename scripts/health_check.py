...
class MemoryHealthChecker:
    ...
    def check_database_status(self) -> Dict:
        ...
        try:
            ...
            try:
                ...
                status["error"] = f"Collection '{COLLECTION_NAME}' not found. Run index_summaries.py first."
        ...
        for query in test_queries:
            try:
                ...
                results.append({
                    ...
                    "top_match": search_results["metadatas"][0][0].get("title", "N/A") if search_results["metadatas"][0] else "None"
                })
        ...
        try:
            ...
            if docs and docs.get("metadatas"):
                ...
                for i, metadata in enumerate(docs["metadatas"]):
                    ...
                    if "semantic_description" in metadata and metadata["semantic_description"]:
    ...
    def generate_report(self) -> None:
        ...
        if db_status["accessible"]:
            ...
            status_text = f"❌ Not accessible: {db_status.get('error', 'Unknown error')}"
        
        console.print(Panel(status_text, title="Database Status", border_style="green" if db_status["accessible"] else "red"))
        ...
        if coverage["missing_summaries"]:
            ...
            if len(coverage["missing_summaries"]) > 5:
                console.print(f"  ... and {len(coverage['missing_summaries']) - 5} more")
        ...
        if model_status["loaded"]:
            ...
            model_text = f"❌ Model not loaded: {model_status.get('error', 'Unknown error')}"
        
        console.print(Panel(model_text, title="Embedding Model Status", border_style="green" if model_status["loaded"] else "red"))
        ...
        if coverage["missing_summaries"]:
            recommendations.append(f"Index {len(coverage['missing_summaries'])} missing summaries by running: python scripts/index_summaries.py")
        
        if coverage["orphaned_entries"]:
            recommendations.append(f"Clean up {len(coverage['orphaned_entries'])} orphaned database entries")
        
        if metadata_quality["total"] > 0:
            low_quality_fields = []
            for field, count in [("title", metadata_quality["with_title"]), 
                                ("date", metadata_quality["with_date"]),
                                ("technologies", metadata_quality["with_technologies"])]:
            ...
            if low_quality_fields:
                recommendations.append(f"Improve metadata for fields: {', '.join(low_quality_fields)}")
